...
from schedule.api import LessonAPI, TeacherAPI
from schedule.utils import generate_teacher_schedule_message
from teacher.keyboard import (
    build_date_change_keyboard,
    build_days_keyboard,
    build_teacher_list_kb,
    build_teacher_schedule_month_keyboard,
    find_teacher_keyboard,
    try_again_name_kb,
)
...
async def get_teacher_list(name: str = ""):
    ...
    if name:
        teachers = [
            ...
            if name.lower() in teacher.get("short_name").lower()
        ]
    return teachers
...
async def get_teacher_schedule(teacher_id: int, date: str):
    ...
    message = await generate_teacher_schedule_message(
        sorted(new_lessons.values(), key=lambda x: x.get("time_from")),
        date,
        teacher,
    )
...
@teacher_router.message(F.text == "🔍 Найти преподавателя")
...
@teacher_router.callback_query(F.data == "teacher_name")
...
@teacher_router.message(FindTeacher.teacher_name)
async def get_teacher_list_by_name(message: types.Message, state: FSMContext):
    ...
    if not teachers:
        await message.answer(
            ...
            reply_markup=try_again_name_kb.as_markup(),
        )
        return
...
@teacher_router.callback_query(F.data.startswith("teacher_list_"))
async def get_full_teacher_list(callback: types.CallbackQuery):
    ...
    kb = await build_teacher_list_kb(
        teachers,
        page=int(page),
...
@teacher_router.callback_query(F.data.startswith("get_teacher_schedule_"))
...
@teacher_router.callback_query(F.data.startswith("teacher_schedule_date_choice"))
...
@teacher_router.callback_query(F.data.startswith("teacher_schedule_month_set"))
