...
find_teacher_keyboard.row(InlineKeyboardButton(
...
try_again_name_kb.add(InlineKeyboardButton(
...
async def build_teacher_list_kb(teachers, page=0, teacher_filter=''):
    ...
    kb.row(InlineKeyboardButton(
        ...
        callback_data=f'teacher_list_{teacher_filter}_{page - 1}' if page > 0 else 'none',
    ))
    ...
    kb.add(InlineKeyboardButton(
        ...
        callback_data=f'teacher_list_{teacher_filter}_{page + 1}' if page < total_pages - 1 else 'none',
    ))
...
async def build_days_keyboard(month_index: int, teacher_id: int):
    ...
    for day in days:
        kb.add(InlineKeyboardButton(
            ...
            callback_data=f'get_teacher_schedule_{teacher_id}_{datetime(year, month_index + 1, day).strftime("%Y-%m-%d")}' if day != 0 else 'none',
        ))
