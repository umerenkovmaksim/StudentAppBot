...
from feedback.api import FeedbackAPI
from feedback.keyboard import build_student_feedbacks_keyboard, feedback_keyboard
...
async def send_feedback(message: types.Message, state: FSMContext):
    ...
    resp = await FeedbackAPI.post(
        json_data={"user_id": user_id, "title": title, "text": text},
    )
