...
from schedule.api import LessonAPI
from schedule.keyboard import (
    build_date_change_keyboard,
    build_days_keyboard,
    month_choice_kb,
)
from schedule.utils import generate_schedule_message
...
async def get_schedule_data(user_id: int, date: datetime):
    ...
    for index in range(len(lessons) - 1, 0, -1):
        if lessons[index].get("time_from") == lessons[index - 1].get("time_from"):
            lessons[index - 1]["cabinet"] += (
                (" | " + lessons[index].get("cabinet"))
                if lessons[index].get("cabinet")
                else ""
            )
            lessons[index - 1]["teacher"]["short_name"] += (
                (" | " + lessons[index]["teacher"]["short_name"])
                if lessons[index].get("teacher")
                else ""
            )
            del lessons[index]
...
async def schedule_date_choice(callback: types.CallbackQuery):
    await callback.message.edit_text(
        "Выберите месяц",
        reply_markup=month_choice_kb.as_markup(),
    )
